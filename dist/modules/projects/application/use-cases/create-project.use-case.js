...
let CreateProjectUseCase = class CreateProjectUseCase {
    ...
    async execute(idWho, idProjectManager, idPlanBoundary, idLayer) {
        if (!idProjectManager || !idPlanBoundary || !idLayer) {
            throw new common_1.BadRequestException('idProjectManager, idPlanBoundary and idLayer are required');
        }
        const id = await this.projectsRepository.createProject(idWho, idProjectManager, idPlanBoundary, idLayer);
        return { id };
    }
};
